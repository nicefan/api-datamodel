...
At runtime, the complete URL is composed as `serverUrl + rootPath + modulePath + requestPath`.

With the default `rootPathSource: 'gateway'`, OpenAPI paths remain unchanged. When it is `document` and `rootPath` exists, the generator removes either the complete `rootPath` or the longest matching suffix prefix from the beginning of the path. See [`rootPathSource`](./config#rootpathsource) for details.
...
- `resource.ts`, which imports the configured Service and exports the corresponding `createApi`.
